...
if (message.action === 'translate') {
    ...
    // Hàm lấy ngôn ngữ với retry
    function getLanguagesWithRetry(retryCount = 0) {
      chrome.storage.sync.get(['targetLanguage', 'sourceLanguage'], function(data) {
        ...
        // Nếu dữ liệu không hợp lệ và chưa vượt quá số lần retry, thử lại
        if ((!data.targetLanguage || !data.sourceLanguage) && retryCount < 5) {
          console.warn('Không lấy được targetLanguage/sourceLanguage, thử lại lần', retryCount + 1);
          setTimeout(() => getLanguagesWithRetry(retryCount + 1), 500);
          return;
        }
    ...
    // Gọi hàm lấy ngôn ngữ với retry
    getLanguagesWithRetry();
